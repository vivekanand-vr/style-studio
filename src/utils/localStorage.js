...
export const searchItems = (items, searchQuery) => {
  ...
  return items.filter((item) => {
    return (
      regexQuery.test(item.title || "") ||
      regexQuery.test(item.brand || "") ||
      regexQuery.test(item.category || "") ||
      regexQuery.test(item.subcategory || "") ||
      regexQuery.test(item.description || "") ||
      regexQuery.test(item.color || "") ||
      regexQuery.test(item.type || "") ||
      regexQuery.test(item.notes || "") ||
      regexQuery.test(String(item.price || ""))
    );
  });
};

export const searchOutfits = (outfits, searchQuery) => {
  if (!searchQuery || searchQuery.trim() === "") {
    return outfits;
  }
  ...
  return outfits.filter((outfit) => {
    return (
      regexQuery.test(outfit.name || "") ||
      regexQuery.test(outfit.description || "") ||
      regexQuery.test(outfit.occasion || "")
    );
  });
};
...
export const saveOutfit = (outfit) => {
  ...
  const newOutfit = {
    ...
    id:
      outfit.id ||
      `outfit_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
...
export const filterItems = (items, filters) => {
  ...
  if (filters.brand) {
    ...
    filtered = filtered.filter(
      (item) => item.brand && item.brand.toLowerCase().includes(brandLower),
    );
  }
